...
<head>
  <meta charset="utf-8">
  <title>Data Kepala Sekolah - SMK Nasional Nganjuk</title>
...
<main class="container py-6">
  <section id="" class="">
  <h1 class="text-center mb-4">Kepala Sekolah SMK Nasional Nganjuk</h1>
    ...
    <div class="row justify-content-center">
      @if($kepala)
        <div class="col-md-6">
          <div class="card shadow">
            @if($kepala->foto)
              <img src="{{ asset('storage/'.$kepala->foto) }}" class="card-img-top" alt="Kepala Sekolah">
            @else
              <img src="{{ asset('assets/img/person/person-m-2.webp') }}" class="card-img-top" alt="Kepala Sekolah">
            @endif
            <div class="card-body text-center">
              <h5 class="card-title">{{ $kepala->nama ?? '-' }}</h5>
              <ul class="list-group list-group-flush mt-3">
                <li class="list-group-item"><strong>NIP/NUPTK:</strong> {{ $kepala->nip_nuptk ?? '-' }}</li>
                <li class="list-group-item"><strong>Tanggal Lahir:</strong> {{ $kepala->tanggal_lahir ?? '-' }}</li>
                <li class="list-group-item"><strong>Jenis Kelamin:</strong> {{ $kepala->jenis_kelamin ?? '-' }}</li>
              </ul>
            </div>
          </div>
        </div>
      @else
        <div class="col-12 text-center">
            <div class="alert alert-info">Belum ada data kepala sekolah yang tersedia.</div>
        </div>
      @endif
    </div>
</main>
